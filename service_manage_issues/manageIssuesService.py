...
@app.get("/")
async def index():
    return {"message": "Managing the Issues"}
...
async def retrive_user_issues(contributer:str, organization:str, repo: str):
  ...
  try:
    ...
    issues_url_year  = f"{URL}?creator={contributer}&since={duration_year}"
    issues_url_month = f"{URL}?creator={contributer}&since={duration_month}"
    issues_url_week  = f"{URL}?creator={contributer}&since={duration_week}"

    response_year = requests.get(issues_url_year, headers={"Authorization": 'token %s' % api_token})
    response_month = requests.get(issues_url_month, headers={"Authorization": 'token %s' % api_token})
    response_week = requests.get(issues_url_week, headers={"Authorization": 'token %s' % api_token})

    if response_year.status_code == 200 and response_month.status_code == 200 and response_week.status_code == 200:
        num_issues_created_year  = len(response_year.json())
        num_issues_created_month = len(response_month.json())
        num_issues_created_week  = len(response_week.json())
